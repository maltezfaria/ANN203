...
question(md""" Let $A$ be an $n \times n$ matrix. Assuming that the product $Ax$ takes $\mathcal{O}(n^2)$ flops and that $A$ is such that the GMRES algorithm converges in $\mathcal{O}(1)$ iterations, estimate the complexity of solving the linear system $Ax=b$ using GMRES. How does that compare to a a direct solver (e.g. using *LU* factorization)?
""",6)
