...
# ╔═╡ a6f326e7-4f91-4a18-830e-b8f10e46631e
...
@inline function apply_hh_matrix!(A,u)
	m,n = size(A)
	@assert length(u) == m
	beta = 2/(u'*u)
...
!!! note "Polynomial fitting"
	...
	```
	that minimizes the following $L_2$ error
	```math
		er = \sum_{i=1}^m |p_n(x_i) - y_i|^2
	```
"""

# ╔═╡ 99c62630-0470-4a7d-a9fd-49e3cf22d29d
md"""
Your next question is to reduce the polynomial fitting problem to something you already know how to solve:
"""

# ╔═╡ 0617bc21-f8e3-4f81-8147-32c8e3e9e8b0
...
# ╔═╡ 4a5ce194-e386-47f6-bef9-45260e73c64e
question(md"""
Given an algorithm to produce the $QR$ factorization of a matrix $A$, explain how to solve the polynomial fitting problem described above. Your algorithm should produce the coefficients $\{ c_i \}_{i=0}^{n}$ of the expansion.
""",9)

# ╔═╡ 1331ab19-31eb-4954-8be6-c21d407bb6c6
question(md"""
Complete the `polynomial_fit` function below to return the best polynomial fit of order `n`, $p_n : \mathbb{R} \to \mathbb{R}$, given the data points $(x_i,y_i), 1 \leq i \leq m$. If your `leastsquares_qr` function is working, use it; otherwise you may use the `qr` function from `LinearAlgebra`.
""",10)

# ╔═╡ 5dcfbfd0-2065-4dec-bfce-d3a94c93b18f
question(md"""If you got it all to work, how does your polynomial fit perform in the example below as you vary the polynomial order $n$ (you can slide the bar to change the value)? Given what you know about linear algebra, what happens to the residue as you increase the order $n$? Is a small residue enough to argue that we have found a good polynomial fit?""","(bonus)")
...
# ╔═╡ a5873b08-021d-43af-a41a-05b1eef8bb92
answer()
...
hint(md"You can use `I` for representing an identity matrix in *Julia*. It is similar to *Matlab*'s `eye`, but is it is *lazy*. Check also `diagm` for a more general method. If you are running this notebook interactively through *Pluto.jl*, check the live docs!")
...
warning(md"To keep things simple, we will implement suboptimal versions of the algorithms studied, focusing mostly on algorithmic details.")
...
check_answer() do
	...
	y = rand(10)
	n = 5
	A = [xi^j for xi in x, j in 0:n]
	p = polynomial_fit(x,y,n)
	@test p(0.1) ≈ evalpoly(0.1,A\y)
end
