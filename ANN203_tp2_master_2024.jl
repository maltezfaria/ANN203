...
md"""!!! danger "Corrected version"
	Not to be released to the students
"""
...
answer(md"""
The figure below shows the expected result. For $A$, the convergence is very fast, with about 13 iterations needed for convergence. With $B$, after $100$ iterations, the error is still above $10^{-3}$. Both matrices have similar condition numbers.

The convergence rate can be estimated using the polynomial
```math
	p_n(z) = \left( 1 - \frac{z}{2} \right)^n
```
Maximizing the value of this polynomial on the disk where the eigenvalues of $A$ lie yields $p_n(z) \approx 4^{-n}$. This is roughly the rate at which we expect GMRES to converge since
```math
	\frac{||r_n||}{||b||} \leq \kappa(V) \inf_{p_n \in P_n} p_n(\Lambda(A)).
```
""")
